...
@connect(
  (state, props) => ({
    config: state.config,
    getSmallGoods: state.getSmallGoods,
  })
)
export default class GroupSlowlySmallAddModal extends Component {
  constructor(props) {
    super(props)
    this.state = {
      loading: false,
  ...
  handleSubmit(e) {
    ...
    this.props.form.validateFields((errors, values) => {
      ...
      this.props.dispatch(fetchSlowlyAddGoods({ ...values, bddwId: this.props.dptId }, (result) => {
      ...
      <div>
        ...
        >
          <Form onSubmit={this.handleSubmit}>
            ...
            >
              {
                getFieldDecorator('wpzl', { rules: [{ required: true, message: '请选择物品种类' }] })(
                  <Select placeholder="">
            ...
            >
              {
                getFieldDecorator('cyrsfz', {
                  rules: [
                    { required: true, message: '请输入持有人身份证' },
                    { pattern: regExpConfig.IDcard, message: '请输入正确的持有人身份证' },
                  ],
                })(
                  <Input placeholder="" />
                )
